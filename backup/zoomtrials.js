...
//function initMap(){
    ...
    var featureLi = document.getElementsByClassName ("feature");
    var div = document.getElementById("polosp1");
    polosp1.addEventListener("click", function(){
      for (var i =0; i < featureLi.length; i++){
        featureLi[i]
      }
      console.log("clicked")
    })
    ...
    polosp1.addEventListener("click", function(){
      geojson.forEach(function(e){
        var li = addEventListener("mousemove", function(){
          console.log(li)
        })
      })
    })
    ...
    var naruto = L.geoJSON.filter(function(e){
    ...
    function geoJSONFilter(){
      var res = geojson.filter(function(e){
    ...
    var re = geojson.filter(function(e){
      console.log(e.coordinates);
      return true;
    })
    
    
    var geojson = L.geoJson(polosp1.geojson, {filter: coordinatesFilter}).addTo(map);
    
    function coordinatesFilter(FeatureCollection) {
      if (Feature.properties.coordinates === true){
        return true;
      }
    }
    var coordinatesFilter();
    console.log(coordinatesFilter);
    var res = geojson.filter(function(coordinates, arr){
      for (var i = 0; i <arguments.length; i++){
        if(coordinates.indexOf(arr) === -1){
          return false;
        }else{
          return true;
        }
